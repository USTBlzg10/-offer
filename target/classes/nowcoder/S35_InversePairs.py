# class InversePairs:
#     def inversePairs(self, arr):
#         length = len(arr)
#         copy = array.copy()
#         return self.inversePairsCore(arr, copy, 0, length - 1)
#
#     def inversePairsCore(self, arr, copy, start, end):
#         if start == end:
#             return 0
#         mid = (start + end) / 2
#         left = self.inversePairsCore(copy, arr, start, mid)
#         right = self.inversePairsCore(copy, arr, mid + 1, end)
#         count = 0
#         i = mid
#         j = end
#         indexcopy = end
#         while i >= start and j >= mid + 1:
#             if array[i] > array[j]:
#                 count += j - mid
#                 copy[indexcopy] = array[i]
#                 indexcopy -= 1
#                 i -= 1
#                 if count > 1000000007:
#                     count %= 1000000007
#             else:
#                 copy[indexcopy] = array[j]
#                 indexcopy -= 1
#                 j -= 1
#         for i in range(i, start - 1, -1):
#             copy[indexcopy] = array[i]
#             indexcopy -= 1
#         for j in range(i, mid, -1):
#             copy[indexcopy] = array[j]
#             indexcopy -= 1
#         return (left + right + count) % 1000000007
#
# if __name__ == '__main__':
#     test = InversePairs()
#     # array = [364, 637, 341, 406, 747, 995, 234, 971, 571, 219, 993, 407, 416, 366, 315, 301, 601, 650, 418,
#     #          355, 460, 505, 360, 965, 516, 648, 727, 667, 465, 849, 455, 181, 486, 149, 588, 233, 144, 174, 557, 67,
#     #          746, 550, 474, 162, 268, 142, 463, 221, 882, 576, 604, 739, 288, 569, 256, 936, 275, 401, 497, 82, 935,
#     #          983, 583, 523, 697, 478, 147, 795, 380, 973, 958, 115, 773, 870, 259, 655, 446, 863, 735, 784, 3, 671,
#     #          433, 630, 425, 930, 64, 266, 235, 187, 284, 665, 874, 80, 45, 848, 38, 811, 267, 575]
#     array = [1, 2, 3, 4, 5, 6, 7, 0]
#     print(test.inversePairs(array))
